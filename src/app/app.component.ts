...
import { TelemetryService, ITelemetryContext } from './modules/telemetry';
import {
...
export class AppComponent implements OnInit, OnDestroy {
  @ViewChild('frameWorkPopUp') frameWorkPopUp:any;
  /**
  ...
  */
    ...
    forkJoin([deviceRegister, custodianOrgDetails]).subscribe((res) => {
      ...
      zip(this.tenantService.tenantData$, this.getOrgDetails(false)).subscribe((res) => {
        if (_.get(res[0], 'tenantData')) {
          const orgDetailsFromSlug ="sunbird" //this.cacheService.get('orgDetailsFromSlug');
  ...
  public checkFrameworkSelected() {
    ...
    this.zone.run(() => {
      const frameWorkPopUp: boolean =true //this.cacheService.get('showFrameWorkPopUp');
      if (frameWorkPopUp) {
