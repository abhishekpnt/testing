...
export class ExplorePageComponent implements OnInit, OnDestroy, AfterViewInit {
            ...
            .pipe(
                ...
                switchMap(currentPageData => {
                    ...
                    if (_.get(currentPageData, 'contentType') === 'explore') {
                        ...
                        if (!this.isUserLoggedIn() && get(this.selectedFilters, 'channel')) {
    ...
    private getContentSection(section, searchOptions) {
        ...
        const requiredPropsArray=[...requiredProps]
        if (_.has(sectionFilters, requiredPropsArray) && searchOptions.filters) {
    ...
    getPersistFilters(defaultFilters?) {
        if (true) {
           // const _filter = this.cacheService.get('searchFilters');
            if (defaultFilters) {
                return {
                    board: this.isUserLoggedIn() ? _.get(this.userService.defaultFrameworkFilters, 'board') :  'board',
